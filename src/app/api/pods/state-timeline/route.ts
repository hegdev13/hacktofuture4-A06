...
import { z } from "zod";
import { getPodsStateTimeline } from "@/lib/observability/metrics";
...
const QuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(100).max(20000).optional(),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await getPodsStateTimeline(parsed.data.endpoint, {
