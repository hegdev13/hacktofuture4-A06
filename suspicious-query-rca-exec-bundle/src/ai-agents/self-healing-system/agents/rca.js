...
class RCAAgent {
  ...
  performRCA(clusterState, detectedIssues) {
    ...
    if (!triggerCheck.triggered) {
      ...
      const output = {
        ...
        rootCauseType: 'none',
        failureType: reportUpdate.issues[0]?.failureType || null,
        failureChain: [],
    ...
    const suspiciousQueryIncident = this.detectSuspiciousQueryIncident(nodeSignals, graph);

    const roots = suspiciousQueryIncident?.rootCause
      ? [suspiciousQueryIncident.rootCause]
      : this.findRootCauses(graph, nodeSignals);

    const rootIssues = roots.map((rootName) =>
      this.composeIssueRecord(rootName, graph, nodeSignals, clusterState || {}, suspiciousQueryIncident)
    );
    ...
    const output = {
      ...
      failureType: primary?.failureType || null,
  ...
  composeIssueRecord(rootName, graph, nodeSignals, clusterState, specialIncident = null) {
    ...
    const failureType =
      specialIncident && specialIncident.rootCause === rootName
        ? 'SuspiciousQueryPattern'
        : this.detectFailureType(rootSignal);
    ...
    return {
      ...
      resolvedAt: null,
      confidence,
      reasoning: this.buildReasoning(rootName, failureType, affectedNodes, specialIncident),
    };
  }

  detectSuspiciousQueryIncident(nodeSignals, graph) {
    const patternRegexes = [
      /query\s+error/i,
      /invalid\s+input/i,
      /unexpected\s+query\s+structure/i,
      /database\s+error/i,
    ];

    const hasPatternEvidence = Array.from(nodeSignals.values()).some((signal) => {
      const corpus = [...(signal.logs || []), ...(signal.events || [])].join(' ');
      return patternRegexes.some((rx) => rx.test(corpus));
    });

    const hasMetricEvidence = Array.from(nodeSignals.values()).some((signal) => {
      const errorRate = Number(signal.metrics?.errorRate || 0);
      const latency = Number(signal.metrics?.latency || 0);
      return (
    ...
    if (!hasPatternEvidence || !hasMetricEvidence) return null;

    const nodeNames = Array.from(graph.nodes);
    let rootCause = nodeNames.find((n) => n === 'db-service') || null;
    if (!rootCause) {
      rootCause = nodeNames.find((n) => /db|database|postgres|mysql|mongo|redis/i.test(n)) || null;
    }
    ...
    return {
      rootCause,
      failureType: 'SuspiciousQueryPattern',
      reasoning: 'Detected suspicious query error patterns in logs with concurrent high error rate and latency.',
    };
  }
  ...
  buildReasoning(rootCause, failureType, affectedNodes, specialIncident = null) {
    if (failureType === 'SuspiciousQueryPattern') {
      const suffix = affectedNodes.length
        ? ` and impacts ${affectedNodes.join(', ')}`
        : '';
      return `${specialIncident?.reasoning || 'Suspicious query behavior detected'} Root cause set to ${rootCause}${suffix}.`;
    }
