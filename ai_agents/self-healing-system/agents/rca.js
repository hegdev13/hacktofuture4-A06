...
class RCAAgent {
  ...
  traceFailureChain(startNode, graph, clusterState) {
    ...
    while (queue.length > 0) {
      ...
      // If unhealthy, check its dependencies
      if (!health.healthy || depth === 0) {
    ...
    // Sort by health status (unhealthy first) then by depth
  ...
  identifyRootCause(chain, issue, clusterState) {
    // Find first unhealthy node in chain (deepest dependency)
    const unhealthy = chain.filter(c => !c.health.healthy);

    if (unhealthy.length > 0) {
      // Return the one with highest depth (deepest dependency)
      const deepest = unhealthy.reduce((max, c) =>
        c.depth > max.depth ? c : max
      );
  ...
  calculateConfidence(chain, rootCause, issue) {
    ...
    // Higher confidence if we found deeper dependencies
    if (rootCause.depth > 0) {
      confidence += Math.min(30, rootCause.depth * 10);
    }

    // Higher confidence if chain has unhealthy nodes
    const unhealthyCount = chain.filter(c => !c.health.healthy).length;
    confidence += Math.min(20, unhealthyCount * 5);
